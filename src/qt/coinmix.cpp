...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::TRCN)
    {
    }
...
void Coinmix::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("TRCN")
    //updateDisplayUnit();
}
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeTRCNAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTRCNAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeTRCNAmount = strAnonymizeTRCNAmount.remove(strAnonymizeTRCNAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTRCNAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeTRCNAmount * COIN) nMaxToAnonymize = nAnonymizeTRCNAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeTRCNAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeTRCNAmount));
        strAnonymizeTRCNAmount = strAnonymizeTRCNAmount.remove(strAnonymizeTRCNAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTRCNAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
