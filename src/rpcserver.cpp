...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop TourCoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "TourCoin server stopping";
}
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        /* TRCN features */
        {"tourcoin", "masternode", &masternode, true, true, false},
        {"tourcoin", "masternodelist", &masternodelist, true, true, false},
        {"tourcoin", "mnbudget", &mnbudget, true, true, false},
        {"tourcoin", "mnbudgetvoteraw", &mnbudgetvoteraw, true, true, false},
        {"tourcoin", "mnfinalbudget", &mnfinalbudget, true, true, false},
        {"tourcoin", "mnsync", &mnsync, true, true, false},
        {"tourcoin", "spork", &spork, true, true, false},
#ifdef ENABLE_WALLET
        {"tourcoin", "Darksend", &Darksend, false, false, true}, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
    ...
    if (((mapArgs["-rpcpassword"] == "") ||
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
                                             _("To use tourcoind, or the -server option to tourcoin-qt, you must set an rpcpassword in the configuration file:\n"
                                               "%s\n"
...
{
    return "> tourcoin-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(string methodname, string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:36221/\n";
}
